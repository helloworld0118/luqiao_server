...
public class LogInterceptor implements HandlerInterceptor {  
    ...
    private String getParamString(Map<String, String[]> map) {  
        ...
        for(Entry<String,String[]> e:map.entrySet()){  
        	if(e.getKey().equals("password")) continue;
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {  
        ...
        if (handler instanceof HandlerMethod) {  
            ...
            sb.append("Method    : ").append(h.getMethod().getName()+"===="+request.getMethod()).append("\n");  
            request.setAttribute("Method", h.getMethod().getName());  
    	    sb.append("Params    : ").append(getParamString(request.getParameterMap())).append("\n");  
            sb.append("URI       : ").append(request.getRequestURI()).append("\n");  
            sb.append("USER      : ").append(Utils.getCurrentUserInfo(request)).append("\n");  
    ...
    public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler, ModelAndView modelAndView) throws Exception {  
        ...
        if(handler instanceof HandlerMethod){  
            ...
            sb.append(new SimpleDateFormat("hh:mm:ss.SSS").format(startTime)+"--------------------------"+request.getAttribute("Method")+"----------------------------------------------------"+new SimpleDateFormat("hh:mm:ss.SSS").format(endTime));  
            LOGGER.error(sb.toString());  
        }  
    }  
    ...
    public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) throws Exception {  
        ...
        if (LOGGER.isDebugEnabled()){  
            ...
            if(ex != null){  
                LOGGER.error("Controller异常: " + getStackTraceAsString(ex));  
            }  
  
            LOGGER.error("计时结束："+ new SimpleDateFormat("hh:mm:ss.SSS").format(endTime) + " 耗时：" + (endTime - beginTime) + " URI:" +  
